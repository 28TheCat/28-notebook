...
- TCP = **每个客户端独立线程**
- UDP = **循环接收 + 线程池处理数据包我帮你把 **Java 集合类**整理成一个 **清晰对照表**，方便快速记忆顺序、特点和常用类。
...
Lambda 表达式是 **Java 8 引入的**，用于简化**只有一个抽象方法的接口**（函数式接口）的匿名内部类写法，使代码更简洁。
...
Stream 是 Java 8 提供的对集合进行“链式操作”的工具，可实现**过滤、转换、聚合、统计**等操作。
Stream 分为三类操作：
